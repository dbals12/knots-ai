...
import {
  AlertDialog,
  AlertDialogAction,import { useEffect, useState, useRef } from "react";
import { useParams, useNavigate, useLocation } from "react-router-dom";
import { supabase } from "@/integrations/supabase/client";
import { useAuth } from "@/contexts/AuthContext";
import { useToast } from "@/hooks/use-toast";
import AppShell from "@/components/AppShell";
import { Button } from "@/components/ui/button";
import { Loader2, RefreshCw } from "lucide-react"; // 새로고침 아이콘 추가
import { SiNaver, SiLinkedin, SiInstagram, SiThreads } from "react-icons/si";
import ResultDetailModal from "@/components/ResultDetailModal";
import {
  AlertDialog,
...
const getSummary = (content: string | null) => {
  ...
  const cleanContent = content.replace(/```json\s*/gi, "").replace(/```\s*/gi, "").trim();
  try {
    const parsed = JSON.parse(cleanContent);
    if (typeof parsed === "object" && parsed !== null) {
      return (parsed["Slide 1"] || parsed["slide 1"] || parsed["Caption"] || parsed["caption"] || content).substring(0, 100) + "...";
    }
  } catch {}
...
const DraftResult = () => {
  ...
  const [showManualRefresh, setShowManualRefresh] = useState(false); // ✅ 수동 새로고침 버튼 표시 여부
  ...
  // ✅ 데이터 확인 함수 (재사용을 위해 분리)
  const checkData = async () => {
    try {
      if (isSessionType) {
        // 회원: Sessions & Outputs 확인
        ...
        if (session) {
            ...
            // 텍스트가 있거나, 결과가 하나라도 있으면 로딩 끝
            if (session.raw_text || Object.keys(result_data).length > 0) {
                setData({ input_text: session.raw_text || "변환된 텍스트가 없습니다.", input_mode: session.input_type, result_data });
                setLoading(false);
                return true;
            }
        }
  ...
  useEffect(() => {
    ...
    checkData().then((done) => {
        if (!done) {
            ...
            // 3. ✅ [안전장치] 5초가 지나도 안 나오면 "수동 버튼" 보여주기
            setTimeout(() => {
    ...
    const channel = supabase.channel(`any-${draftId}`)
  ...
  useEffect(() => {
    ...
    const runAI = async () => {
      ...
      try {
        ...
        const response = await fetch(`https://qdzhwrcanenolbocysmx.supabase.co/functions/v1/process-audio`, { method: "POST", body: formData });
        if (!response.ok) throw new Error("AI Processing Failed");
        const aiResult = await response.json();

        const updatedInputData = { ...inputData, textInput: aiResult.transcript || inputData.textInput };
        await supabase.from("drafts").update({ status: "completed", result_data: aiResult.content, input_data: updatedInputData }).eq("id", draftId);
      } catch (error) {
        ...
        if (user && pendingId && pendingId === draftId && !isSessionType) {
            ...
            try {
                ...
                if (draft) {
                    ...
                    const rd = resultData || {};
                    if (rd.blog_content) outputsToInsert.push({ session_id: session.id, platform_type: "blog", generated_content: rd.blog_content });
                    if (rd.linkedin_content) outputsToInsert.push({ session_id: session.id, platform_type: "linkedin", generated_content: rd.linkedin_content });
                    if (rd.reels_content) outputsToInsert.push({ session_id: session.id, platform_type: "reels", generated_content: rd.reels_content });
                    if (rd.threads_content) outputsToInsert.push({ session_id: session.id, platform_type: "threads", generated_content: rd.threads_content });
  ...
  const performLogin = () => { navigate(`/login?next=/result/${draftId}?type=draft`); };
  const handleEditInput = () => { if (!user) { setShowLoginAlert(true); return; } navigate("/input", { state: { initialText: data?.input_text } }); };
  const handleCopyAction = (content: string) => { if (!user) { setShowLoginAlert(true); return; } navigator.clipboard.writeText(content).then(() => { toast({ title: "복사 완료", description: "클립보드에 복사되었습니다." }); }); };
  const handleContentUpdate = async (newContent: string) => { if (!selectedPlatform || !data) return; const updatedResult = { ...data.result_data }; if (selectedPlatform === "blog") updatedResult.blog_content = newContent; else if (selectedPlatform === "linkedin") updatedResult.linkedin_content = newContent; else if (selectedPlatform === "reels") updatedResult.reels_content = newContent; else if (selectedPlatform === "threads") updatedResult.threads_content = newContent; setData({ ...data, result_data: updatedResult }); if (isSessionType && user) { await supabase.from("outputs").update({ generated_content: newContent }).eq("session_id", draftId).eq("platform_type", selectedPlatform); } };
  const handleCardClick = (platformKey: string) => { setSelectedPlatform(platformKey); setIsModalOpen(true); };
  const getContent = (key: string) => { if (!data?.result_data) return ""; const rd = data.result_data; if (key === "blog") return rd.blog_content; if (key === "linkedin") return rd.linkedin_content; if (key === "reels") return rd.reels_content; if (key === "threads") return rd.threads_content; return ""; };

  // ✅ 로딩 화면 (타임아웃 시 수동 버튼 표시)
